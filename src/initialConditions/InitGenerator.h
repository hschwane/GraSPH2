...
{
    static_assert(std::is_base_of<ParticleSourceType,ParticleSourceBaseFlag>::value, "particleSource must be an object of a class derived from the ParticleSource class!");
...
hostParticleType InitGenerator<hostParticleType>::generate()
{
    ...
    for(const auto &item : m_genFuncs)
    {
        ...
        for(int i = std::get<0>(item); i < std::get<1>(item); ++i)
        {
            // generate particle i
            std::get<2>(item)(hpb);
        }
