...
private:
    using sourceListT = std::tuple<size_t,size_t,std::function<void>(hostParticleType&,size_t)>;
    std::vector<sourceListT> m_genFuncs; //!< vector containing all particle sources
...
{
    static_assert(std::is_base_of<ParticleSourceType,detail::ParticleSourceBaseFlag>::value, "particleSource must be an object of a class derived from the ParticleSource class!");

    size_t numParticles = particleSource.getNumParticles();
    m_genFuncs.emplace_back(m_totalNumOfParticles,m_totalNumOfParticles+numParticles,particleSource);
...
hostParticleType InitGenerator<hostParticleType>::generate()
{
    ...
    for(const auto &item : m_genFuncs)
    {
        // all particles that need to be generated by this source
        for(int i = std::get<0>(item), j=0; i < std::get<1>(item); ++i, ++j)
        {
            // generate particle i
            std::get<2>(item)(hpb, j,i);
        }
    }
    return hpb;
}
