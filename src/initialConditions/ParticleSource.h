...
public:
    ...
    size_t getNumParticles() const { return m_numberOfParticles; } //!< the number of particles provided by this source
    f3_t getDesiredCOM() const { return m_desiredCOM;} //!< returns the desired com of the particle system generated by this source, assuming the origin as initial value and transforming according to all applied modifiers
    f3_t getDesiredVelOfCOM() const {return m_desiredVelOfCOM;} //!< returns the desired velocity of the com of the particle system generated by this source, assuming the origin as initial value and transforming according to all applied modifiers
...
Derived &ParticleSource<particleType,Derived>::addAngularVelocity(f3_t omega)
{
    ...
    m_desiredVelOfCOM += cross(omega,m_desiredCOM);
    return *static_cast<Derived*>(this);
}
...
Derived &ParticleSource<particleType, Derived>::rotate(f3_t axis, f1_t angle)
{
    ...
    m_desiredCOM = R * m_desiredCOM;
    m_desiredVelOfCOM = R * m_desiredVelOfCOM;
    return *static_cast<Derived*>(this);
}
