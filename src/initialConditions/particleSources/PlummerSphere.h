...
{
public:
    PlummerSphere(size_t particleCount, f1_t plummerRadius, f1_t cutoffRadius, f1_t totalMass, f1_t materialDensity,
                  unsigned int seed = mpu::getRanndomSeed());
    ~PlummerSphere() override = default;

    PlummerSphere &addRandomPlummerVelocity(f1_t G = 1, unsigned int seed = mpu::getRanndomSeed()); //! Add random velocity according to plummer distribution to each particle. Cloud will end up in dynamic equilibrium.

private:
    Particle<POS, MASS, DENSITY> generateParticle(size_t id) override;

    const f1_t m_plummerRadius;
    const f1_t m_cutoffRadius;
    const f1_t m_matDensity;
    const f1_t m_totalMass;
    const f1_t m_particleMass;
